...
export async function fetchTextCompletion(prompt, language, context) {
    const systemPrompt = `You are a multilingual translator, fluent in local slang and tone variation. Your task is to translate the user input into a specified language, using tone, dialect and audience as context.
    ...
    const userPrompt = `
                ...
                Use vocabulary, phrasing, and expressions typical of the following regional dialect:
                - Dialect: ${context.dialect || 'Standard'}
                
                ${context.dialect && dialectHints[context.dialect] ? `Note: ${dialectHints[context.dialect]}` : ''}
                
                Audience: ${context?.audience || 'General'}
                Intent: ${context?.goal || 'General'}
